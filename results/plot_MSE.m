...
% -----------------------------------------------------
function plot_MSE(figID, runID, curves, legends)
...
for curve_idx = 1:n_curves
    ...
    % normalize results
    res.MSE = res.MSE./res.num_trials;
    ...
    figure(figID)   
    if curve_idx==1
        semilogy(par.SNRdB_list,res.MSE,marker_style{curve_idx},'LineWidth',2, 'DisplayName', legends{curve_idx});
        hold on
    else
        semilogy(par.SNRdB_list,res.MSE,marker_style{curve_idx},'LineWidth',2, 'DisplayName', legends{curve_idx});
    end
...
set(gca,'FontSize',12)
savefig([my_path 'figures/' 'MSE_' plot_title])
